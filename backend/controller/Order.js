...
exports.placeOrder = async (req, res) => {
  try {
    ...
    // Validate payment method - FIXED: Check if it's a string
    ...
    // Calculate totals - FIXED: Match Order model requirements
    ...
    // Apply promo code
    if (promoCode && promoCode.trim() !== "") {
      const promoResult = validatePromoCode(promoCode.trim(), itemsTotal, shippingCost);
      if (promoResult.valid) {
        discount = promoResult.discount;
      }
    }

    const taxAmount = 0; // FIXED: Match model field name
    const totalAmount = itemsTotal + shippingCost + taxAmount - discount;

    // Generate unique order number - FIXED: Required field
    const timestamp = Date.now();
    const randomStr = Math.random().toString(36).substr(2, 6).toUpperCase();
    const orderNumber = `ORD-${timestamp}-${randomStr}`;

    // Create order - FIXED: Match Order model schema exactly
    const newOrder = new Order({
      ...
      paymentMethod: {
        type: paymentMethod, // FIXED: Should be string value
        details: {}
      },
      orderSummary: {
        itemsTotal: Math.round(itemsTotal), // FIXED: Required field
        shippingCost: Math.round(shippingCost),
        taxAmount: Math.round(taxAmount), // FIXED: Required field
        discount: Math.round(discount),
        totalAmount: Math.round(totalAmount)
      },
      orderNumber: orderNumber, // FIXED: Required field
      notes: notes || "",
      trackingNumber: orderNumber, // Optional: if you want both
      promoCode: promoCode || null,
      orderStatus: "pending",
      isPaid: paymentMethod === 'cash_on_delivery' ? false : true
    });
